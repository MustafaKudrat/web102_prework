...
// zoohowcedar
        ...
        <h2>${game.name}</h2>
        <p>Goal to raise: ${game.goal}</p>
        <p>Raised so far: ${game.pledged}</p>
        `;
...
function filterUnfundedOnly() {
    ...
    const notMetGames = GAMES_JSON.filter(game => game.pledged < game.goal);
...
function filterFundedOnly() {
    ...
    const metGames = GAMES_JSON.filter(game => game.pledged >= game.goal);
...
const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for 4 games. Currently, 
${unfundedGames} game${unfundedGames === 1 ? "" : "s"} remain unfunded. 
...
firstGameElement.innerText = firstGameName;
...
secondGameElement.innerText = secondGameName;
